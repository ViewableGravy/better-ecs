...
function getPlayerAnimationFrames(
  ...
  return PLAYER_FRAME_INDICES.map((frame) => {
    // State, direction, and frame are constrained to registered player sheet keys.
...
export function createPlayerSprite(
  ...
  sprite.zOrder = previousSprite?.zOrder ?? derivePlayerSpriteZOrder(0);
  sprite.playbackRate = PLAYER_PLAYBACK_RATE_BY_STATE[animationState];

  return sprite;
}

export function derivePlayerSpriteZOrder(worldY: number): number {
