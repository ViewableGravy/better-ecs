...
describe("SceneManager", () => {
  ...
  it("should notify subscribers when transitioning scenes", async () => {
    const MenuScene = createScene("menu")({
      setup: () => {
        /* setup */
      },
    ...
    const unsubscribe = engine.scene.subscribe(listener);

    await engine.scene.set("menu");

    expect(listener).toHaveBeenCalled();
    expect(engine.scene.activeSceneName).toBe("menu");

    unsubscribe();
  });

  it("should notify subscribers when active world changes", async () => {
    const MenuScene = createScene("menu")({
      setup: () => {
        /* setup */
      },
      sceneSetup: (context) => {
        context.loadAdditionalWorld("overworld");
      },
    });
    ...
    engine.scene.setActiveWorld("overworld");

    expect(listener).toHaveBeenCalledTimes(1);
    expect(engine.scene.activeWorldId).toBe("overworld");
