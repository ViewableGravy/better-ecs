 ...
 */
function internalMatchKeybind(state: InputState, bind: KeyBind | undefined, mode: 'pressed' | 'down' | 'released' = 'pressed'): boolean {
 ...
 * @returns true if the keybind matches
 */
export function matchKeybindString(bindStr: string | undefined, state: InputState, mode: 'pressed' | 'down' | 'released' = 'pressed'): boolean {
...
type MatchKeybindReturn<TType extends Type, TReturn extends ReturnMode> = 
...
export function createMatchKeybind(state: InputState) {
  ...
  function matchKeybind<TType extends "some" | "every" | undefined = undefined, TReturn extends ReturnMode = "boolean">(
    options: matchKeybindOptions<TType, TReturn>
  ): MatchKeybindReturn<TType, TReturn>;
  function matchKeybind(optionsOrBind?: matchKeybindOptions<Type, ReturnMode> | KeyBind | KeyBindGroup): any {
    ...
    if ('code' in (optionsOrBind as KeyBind)) {
      ...
      if (Array.isArray(bind.code)) {
        return bind.code.some((code) => internalMatchKeybind(state, { code, modifiers: bind.modifiers }, 'pressed'));
      }
